...
<c:url var="updatePlanUrl" value="/UpdatePlanServlet" />
...
<div class="qfit-page-wrapper">
    ...
    <header class="qfit-nav">
        ...
        <nav class="qfit-dashboard-nav">
            <a href="${planDetailsUrl}" class="qfit-btn qfit-btn-secondary">Cancel</a>
    ...
    <main class="qfit-detail-main">
        <c:choose>
            <c:when test="${not empty plan}">
                ...
                <form method="post" action="${updatePlanUrl}" id="editPlanForm" class="qfit-modal-form qfit-glass-card">
                    ...
                    <div class="qfit-form-group">
                        ...
                        <input type="color" id="planColor" name="planColor"
                               value="<c:out value='${plan.color}' default='#5b78ff'/>"
                               required>
                    </div>
                    ...
                    <div class="qfit-form-group">
                        ...
                        <div class="qfit-day-checkbox-group">
                            <input type="checkbox" id="mon" name="days" value="MON" <c:if test="${fn:contains(days,'MON')}">checked</c:if>><label for="mon">M</label>
                            <input type="checkbox" id="tue" name="days" value="TUE" <c:if test="${fn:contains(days,'TUE')}">checked</c:if>><label for="tue">T</label>
                            <input type="checkbox" id="wed" name="days" value="WED" <c:if test="${fn:contains(days,'WED')}">checked</c:if>><label for="wed">W</label>
                            <input type="checkbox" id="thu" name="days" value="THU" <c:if test="${fn:contains(days,'THU')}">checked</c:if>><label for="thu">T</label>
                            <input type="checkbox" id="fri" name="days" value="FRI" <c:if test="${fn:contains(days,'FRI')}">checked</c:if>><label for="fri">F</label>
                            <input type="checkbox" id="sat" name="days" value="SAT" <c:if test="${fn:contains(days,'SAT')}">checked</c:if>><label for="sat">S</label>
                            <input type="checkbox" id="sun" name="days" value="SUN" <c:if test="${fn:contains(days,'SUN')}">checked</c:if>><label for="sun">S</label>
                        </div>
                    </div>
                            ...
                            <div class="qfit-exercise-group">
                                ...
                                <div class="qfit-exercise-row">
                                    <div class="qfit-form-group">
                                        ...
                                        <input type="text"
                                               id="exName${loop.index + 1}"
                                               name="exName${loop.index + 1}"
                                    ...
                                    <div class="qfit-form-group qfit-small-input">
                                        ...
                                        <input type="number"
                                               id="exSets${loop.index + 1}"
                                               name="exSets${loop.index + 1}"
                                               min="1"
                                               value="<c:out value='${exercise.sets}' default='3'/>"
                                               required>
                                    </div>
                                    ...
                                    <div class="qfit-form-group qfit-small-input">
                                        ...
                                        <input type="number"
                                               id="exWeight${loop.index + 1}"
                                               name="exWeight${loop.index + 1}"
                                               min="0"
                                               value="<c:out value='${exercise.weightLbs}' default='0'/>"
                                               required>
                                    </div>
                                    ...
                                    <div class="qfit-form-group qfit-small-input">
                                        ...
                                        <input type="number"
                                               id="exTime${loop.index + 1}"
                                               name="exTime${loop.index + 1}"
                                               min="0"
                                               value="<c:out value='${exercise.timeMinutes}' default='0'/>"
                                               required>
                                    </div>

                                    <button type="button"
                                            class="qfit-btn qfit-btn-small qfit-remove-btn"
                                            onclick="this.closest('.qfit-exercise-group').remove()">
                                        X
                                    </button>
                                </div>

                                <input type="hidden" name="exerciseId${loop.index + 1}" value="<c:out value='${exercise.id}'/>">
                            </div>
                        </c:forEach>
                    </div>

                    <button type="button" class="qfit-btn qfit-btn-secondary qfit-btn-sm" onclick="addExerciseField()">
                        + Add New Exercise
                    </button>

                    <br><br>

                    <button type="submit" class="qfit-btn qfit-btn-primary qfit-full-width">
                        Save Changes
                    </button>
                </form>

                <button class="qfit-btn qfit-btn-ghost qfit-full-width"
                        style="margin-top: 15px; color: #ff6b81;"
            ...
            <c:otherwise>
                <div class="qfit-status-card qfit-glass-card">
                    ...
                    <a href="${dashboardUrl}" class="qfit-btn qfit-btn-primary">Go to Dashboard</a>
                </div>
            </c:otherwise>
        </c:choose>
    </main>
...
<script>
    ...
    let exerciseCount = ${fn:length(plan.exercises)};
    ...
    function addExerciseField() {
        ...
        // New exercises get placeholder ID "new"
        exerciseGroup.innerHTML = `
            ...
            <div class="qfit-exercise-row">
                ...
                <button type="button" class="qfit-btn qfit-btn-small qfit-remove-btn"
                        onclick="this.closest('.qfit-exercise-group').remove()">X</button>
            </div>
    ...
    function confirmDelete() {
        ...
        // Reuse the same form + servlet, just switch action to delete
